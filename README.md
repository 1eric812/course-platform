...
`GET /api/status` 另返回 `phases`（来自 `selection_period` 表），驱动首页「选课节点」卡片；
该表未初始化时 `phases` 返回空数组、卡片自动隐藏，接口不因缺表报错。

服务启动后 **90 秒** 模拟「选课开放」，用于演示倒计时与抢课；`StreamController` 每 4 秒
随机消耗若干名额并通过 SSE 推送，模拟高峰期余量变化。教务端提供「重置演示数据」：
清空课程活动数据并重新灌入（账号与规则保留）。
