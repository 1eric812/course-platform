...
export async function renderWishlist(view) {
  ...
  view.innerHTML = `
    ...
    ${items.length ? `
      <div class="card" style="display:flex;justify-content:space-between;align-items:center;flex-wrap:wrap;gap:10px">
        ...
        <button class="btn primary" id="submitAll">${icon("send", 18)} 一键提交选课</button>
      </div>
      <div id="wlList"></div>
  ...
  if (submitBtn) submitBtn.onclick = async () => {
    ...
    if (r.data.error) { toast("提交失败", r.data.error, "danger"); submitBtn.disabled = false; return; }
...
export async function renderSettings(view) {
  ...
  view.querySelectorAll("[data-switch]").forEach((b) => (b.onclick = async () => {
    const r = await api.switchAccount(b.dataset.switch);
    if (r.data.error) { toast("切换失败", r.data.error, "danger"); return; }
    toast("已切换账号", `当前账号：${r.data.user.name}`, "ok");
...
export async function renderTeacher(view) {
  ...
  view.querySelectorAll("[data-export]").forEach((b) => (b.onclick = () => {
    ...
    window.open(`/api/teacher/courses/${b.dataset.export}/roster.csv`, "_blank");
  }));
...
export async function renderAdminMonitor(view) {
  ...
  async function paint() {
    ...
    set("monUptime", `${Math.floor(m.uptime / 60)} 分 ${m.uptime % 60} 秒 · Node ${m.node}`);
